...
export default	class Robot {
	...
	Robot.Link = class Link {
		constructor(parent, p0, p1, dist, size, light, force, disk) {
			...
			this.shadow = stroke("rgba(0,0,0,.5)");
		}
	};
	...
	const canvas = {
		...
		resize() {
			console.log('canvas高度:'+this.elem.offsetHeight)
			this.width = this.elem.width = this.elem.offsetWidth ? this.elem.offsetWidth : window.innerWidth;
			this.height = this.elem.height = this.elem.offsetHeight ? this.elem.offsetHeight : window.innerHeight;
	...
	const run = () => {
		...
		ctx.fillStyle = "transparent";//此处是绘制上下蒙层
